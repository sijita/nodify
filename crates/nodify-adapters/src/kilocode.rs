//! Adaptador de Kilo Code (CLI). Lee el bloque `mcp` de `~/.config/kilo/kilo.jsonc`
//! (JSONC: admite comentarios y comas colgantes). El formato es casi idéntico al de
//! OpenCode: servidores con `type: local|remote`, `command` como array, `environment`,
//! `url`, `headers`, `enabled`. El modelo vive en el mismo archivo (`"model": "prov/id"`).
...
impl Adapter for KiloCodeAdapter {
    ...
    /// `model` (formato `"provider/model-id"`) en `kilo.jsonc`.
    ...
    fn set_model(&self, raw: &str, model: &str) -> Result<String, AdapterError> {
        ...
        match value_range(raw, "model")? {
            ...
            None => {
                ...
                let map = root
                    .as_object_mut()
                    .ok_or_else(|| invalid("model", "la raíz no es un objeto"))?;
    ...
    /// Kilo no declara proveedores de forma estándar en este archivo.
    fn parse_providers(&self, _raw: &str) -> Vec<nodify_core::ProviderInfo> {
        Vec::new()
    }

    /// Kilo lee las env vars del shell / del propio bloque `environment` del servidor;
    /// no hay un bloque `env` global de settings donde inyectar valores.
    fn set_env(&self, _raw: &str, key: &str, _value: &str) -> Result<String, AdapterError> {
        Err(invalid(
            key,
            "Kilo lee las env vars del shell; defínela ahí o en el 'environment' del MCP",
        ))
    }
}
...
/// Rango de texto `[start,end)` del **valor** de una clave de nivel superior, para
/// reemplazarlo preservando el resto (comentarios incluidos).
...
fn write_mcp_object(raw: &str, mcp_obj: Map<String, Value>) -> Result<String, AdapterError> {
    ...
    match value_range(raw, "mcp")? {
        ...
        None => {
            ...
            let map = root
                .as_object_mut()
                .ok_or_else(|| invalid("mcp", "la raíz no es un objeto"))?;
...
/// Forma nativa Kilo de un MCP canónico (`command` como array, `environment`).
...
fn parse_one(name: &str, cfg: &Value) -> Result<CanonicalMcp, AdapterError> {
    // Se infiere `remote` por presencia de `url` cuando `type` está ausente.
    ...
    let mut mcp = match ty {
        "local" => {
            ...
            m.args = args;
            // Oficial: `environment`. Fallback: `env`.
        ...
        other => return Err(invalid(name, &format!("type desconocido: {other}"))),
    };
...
/// Extrae `command` + `args` aceptando array único o string + `args` separado.
fn split_command(cfg: &Value) -> Result<(String, Vec<String>), AdapterError> {
    match cfg.get("command") {
        Some(Value::Array(arr)) => {
            ...
            let command = parts
                .next()
                .ok_or_else(|| invalid("<mcp>", "command[] vacío"))?;
        ...
        _ => Err(invalid("<mcp>", "transporte local sin 'command'")),
    }
}
