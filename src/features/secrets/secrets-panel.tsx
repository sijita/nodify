...
export function SecretsPanel() {
  ...
  const apply = async (key: string) => {
    ...
    try {
      ...
      setValues((v) => ({ ...v, [key]: "" }));
      setMsg(`${key} → escrito en Claude (settings.env)`);
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-[1180px]">
      ...
      <p className="mb-4 font-sans text-muted-foreground text-xs">
        Variables de entorno referenciadas por MCPs y proveedores. El valor se escribe en
        <strong> Claude</strong> (`settings.env`); Codex/OpenCode las leen del shell o `auth.json`.
        Nodify no guarda el valor (ADR-0004).
      </p>

      <Card>
        {keys.length === 0 && (
          <p className="p-4 text-faint text-xs">{"> sin variables referenciadas"}</p>
        )}
